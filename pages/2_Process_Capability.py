...
def render_capability_charts_tab(manager, filtered_df: pd.DataFrame, deviations_data: pd.DataFrame):
    ...
    with col2:
        # Get default spec limits from config and allow user override for "what-if" analysis
    ...
    # Perform calculations using the analytics engine
    ...
    with plot_col1:
        ...
        # Pass the relevant subset of deviations_data for event overlays
...
def render_anova_tab(filtered_df: pd.DataFrame, cpk_config):
    ...
    if st.button("🔬 Run ANOVA Analysis", type="primary"):
        ...
        if filtered_df[group_col].nunique() < 2:
            ...
            with st.spinner("Performing Analysis of Variance..."):
                ...
                # If ANOVA is significant and there are more than 2 groups, run Tukey's HSD
    ...
    if anova_results:
        ...
        if p_value is not None:
            ...
            with res_col2:
                anova_plot = plotting.plot_anova_results(filtered_df, value_col, group_col, anova_results)
                st.plotly_chart(anova_plot, use_container_width=True)
        else:
    ...
    if tukey_results is not None and not tukey_results.empty:
        ...
        st.info(
            ...
            "If `reject` is `True`, the difference between `group1` and `group2` is statistically significant."
        )
...
# --- Main Page Logic ---
def main():
    """Main function to render the Process Capability Dashboard page."""
