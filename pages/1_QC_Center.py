    ...
    with col1:
        ...
        if submitted:
            with st.spinner("Running QC checks..."):
                try:
                    # Logic is now cleaner: directly call the analytics function
                    report = analytics.apply_qc_rules(
                        df=selected_df,
                        rules_config=rules_config,
                        app_config=manager.settings.app
                    )
                    # Use standard session_state, not a custom manager method
    ...
    with col2:
        ...
        if report_df is None:
            ...
            if st.button("Create Deviation Ticket from Results", type="secondary"):
                try:
                    # Pass username explicitly for audit trail
                    ...
                    # Clear the report so the user can run a new one
...
def render_statistical_dive_tab(selected_df: pd.DataFrame):
    ...
    # Set default to 'purity' if it exists
    default_index = numeric_cols.index('purity') if 'purity' in numeric_cols else 0
    param = st.selectbox("Select Parameter for Statistical Analysis", options=numeric_cols, index=default_index)
...
def render_ml_anomaly_tab(selected_df: pd.DataFrame):
    ...
    with st.form("ml_anomaly_form"):
        ...
        with c1:
            x_col = st.selectbox("X-axis variable", numeric_cols, index=numeric_cols.index('purity') if 'purity' in numeric_cols else 0)
        with c2:
            y_col = st.selectbox("Y-axis variable", numeric_cols, index=numeric_cols.index('bio_activity') if 'bio_activity' in numeric_cols else 1)
        with c3:
            z_col = st.selectbox("Z-axis variable", numeric_cols, index=numeric_cols.index('main_impurity') if 'main_impurity' in numeric_cols else 2)
        with c4:
            contamination = st.slider("Anomaly Sensitivity", 0.01, 0.2, 0.05, 0.01, help="Higher values will flag more points as anomalies.")
        ...
        with st.spinner("Running anomaly detection model..."):
            try:
                ...
                if len(set(cols_to_use)) != 3:
                    ...
                    # Store results in standard session_state
                    st.session_state.ml_results = {
                        'preds': predictions, 'data': data_fitted, 'cols': cols_to_use
                    }
            except Exception as e:
                logger.error(f"Anomaly detection failed: {e}", exc_info=True)
                st.error(f"Failed to run anomaly detection: {e}")

    # Read results directly from session_state
...
# --- Main Page Logic ---
def main():
    """Main function to render the QC & Integrity Center page."""
    ...
    # Use the new centralized, cached data fetching function
