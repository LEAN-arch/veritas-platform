# src/veritas/engine/analytics.py
...
def test_stability_poolability(df: pd.DataFrame, assay: str, time_col: str = 'timepoint_months', group_col: str = 'lot_id') -> Dict:
    ...
    This checks if the slopes of regression lines for different lots are statistically different.

    Args:
        df (pd.DataFrame): Stability data.
        assay (str): Column name of the assay to analyze (e.g., 'purity').
        time_col (str): Column name for the time variable.
        group_col (str): Column name for the grouping variable (e.g., 'lot_id').

    Returns:
        Dict: A dictionary with 'poolable' (bool), 'p_value' (float), and 'reason' (str).
    """
...
def perform_tukey_hsd(df: pd.DataFrame, value_col: str, group_col: str) -> pd.DataFrame:
    ...
    Returns:
        pd.DataFrame: A DataFrame with the pairwise comparison results.
    """
    df_clean = df[[value_col, group_col]].dropna()
...
def run_anomaly_detection(df: pd.DataFrame, cols: List[str], contamination: float, random_state: int = 42) -> Tuple[np.ndarray, pd.DataFrame]:
    ...
    Args:
        ...
        cols (List[str]): List of column names to use for detection.
